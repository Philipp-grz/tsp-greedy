...
CommandLineArgs parse_args(const int count, char ** args) {
   if (good) {
...
void run(CommandLineArgs const & args) {
...
int main(const int count, char ** argv){
   try {
      auto const args = parse_args(count, argv);
      run(args);
      return 0;
   }
   catch (...) {
