...
{																	// Als Parameter übergeben wir Pointer auf die Kanten, die Dimension,
	...
	for (auto const & aktuelle_kante : edges)								// Die (vorher sortierten) Kanten gehen wir nun einmal vollständig durch:
	{
		if (tour_index!=dim)										// Sollte unsere Tour allerdings bereits dim viele Kanten haben, also vollständig sein, brauchen wir nichts weiter zu tun.
		{
            ...
            auto const p2 = n2->_parent->find();
				if ((p1 != p2) or ((p1 == p2) and tour_index==(dim-1))) // Ebenfalls prüfen wir ob die Knoten zusammenhängend sind (mithilfe von Union-Find).
