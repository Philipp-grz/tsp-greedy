...
OPTFLAGS_OPT=-O3 -DNDEBUG -march=native
OPTFLAGS_DEBUG=-O0 -g
...
DEPFILE_USER_HEADERS=-MMD -MP
DEPFILE_ALL_HEADERS=-MD -MP
# set to one of the above
DEPFILE_GENERATION=$(DEPFILE_ALL_HEADERS)
COMPILE_FLAGS=$(DEPFILE_GENERATION) $(WARNING_FLAGS) -pipe $(OPTFLAGS)
# Flags for linking only.
LINK_FLAGS=-fuse-ld=gold
# C / C++ specific compilation flags
CXX_STD_FLAGS=-std=c++1z
...
EXECUTABLE_REL=main
LINK_NAME=$(EXECUTABLE_REL)
EXECUTABLE=$(BUILDDIR)/$(EXECUTABLE_REL)$(LIBEXT)
...
# Mention default target BEFORE including Make.config s.t.
# it is always the first mentioned target, i.e. the default
default:
...
opt debug:
	@$(LN) -sf $$(readlink -f $(EXECUTABLE)) $(LINK_NAME)

check: $(BUILDDIR)
	@forbidden=$$(find $(BUILDDIR) -not -type d $(foreach filext, .o .d .gcno .gcda, -not -name "*$(filext)") $(foreach libext, $(LIBEXTS) "", -not -name "$(EXECUTABLE_REL)$(libext)")); \
...
compile_commands.json: clean
	bear -l /usr/lib64/bear/libear.so $(MAKE) $(MFLAGS) debug
	compdb -p . list > $@.with_headers
	mv $@.with_headers $@
